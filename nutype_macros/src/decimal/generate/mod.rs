    ...
    T: DecimalType + ToTokens + PartialOrd,
{
    ...
    fn gen_fn_sanitize(
        ...
        let transformations: TokenStream = sanitizers
            .iter()
            .map(|san| match san {
                DecimalSanitizer::With(custom_sanitizer) => {
                    quote!(
                        value = (#custom_sanitizer)(value);
                    )
                }
                DecimalSanitizer::_Phantom(_) => {
                    unreachable!("decimal::gen: DecimalSanitizer::_Phantom must not be used")
                }
            })
            .collect();
    ...
    fn gen_fn_validate(
        ...
        let validations: TokenStream = validators
            ...
            .map(|validator| match validator {
                DecimalValidator::Less(exclusive_upper_bound) => {
                    quote!(
                ...
                DecimalValidator::LessOrEqual(max) => {
                    quote!(
                        if val > #max {
                ...
                DecimalValidator::Greater(exclusive_lower_bound) => {
                    quote!(
                ...
                DecimalValidator::GreaterOrEqual(min) => {
                    quote!(
                        if val < #min {
                ...
                DecimalValidator::Predicate(custom_is_valid_fn) => {
                    quote!(
